...
% in circular space with ends wrapping). Smoothing kernel can be either a
% boxcar or gaussian (specified with 'boxcar' or 'gaus').
%
% NOTE SPECIAL CASE TO DEAL WTIH RATEMAP AS INPUT. 
% pxd option in bin_data returns ratemaps direclty - not a seperate pos map
% and spike map. So smoothing has to be applied differently. To just apply 
% the standard smoothing to rate but still mark out unvisted bins with nans
% supply the first variable (binPos) as a logical array of 1s for visited
...
switch kernType
    case 'boxcar'
        kern=ones(kernSize);
    case 'gaus'
    % Define a square kernel with side = 2*kernSize + 1
    halfSize = kernSize;
    [x, y] = meshgrid(-halfSize:halfSize, -halfSize:halfSize);
    sigma = kernSize;
    kern = exp(-(x.^2 + y.^2) / (2*sigma^2));
    kern = kern / sum(kern(:));  % normalize to sum to 1
end
